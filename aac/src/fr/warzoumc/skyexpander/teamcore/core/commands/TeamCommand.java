...
import fr.warzoumc.skyexpander.teamcore.core.team.teamobject.core.level.Donnation;
import fr.warzoumc.skyexpander.teamcore.core.team.teamobject.core.level.PassLevel;
...
import fr.warzoumc.skyexpander.teamcore.core.team.teamobject.core.teamevent.players.Joining;
import fr.warzoumc.skyexpander.teamcore.core.team.teamobject.core.teamevent.players.Leaving;
import fr.warzoumc.skyexpander.teamcore.core.team.teamobject.core.teamevent.players.PlayerTeamInformation;
            ...
            if (args.length == 1){
                ...
                if (args.length == 2){
                    if (args[0].equalsIgnoreCase("create")){
                        ...
                        Team targetTeam = Team.fromPlayerName(main, args[1]);
                        Team senderTeam = Team.fromPlayer(main, player);
                        if (targetTeam != null && senderTeam != null && senderTeam.getTeamName().equalsIgnoreCase(targetTeam.getTeamName())) {
                            new PlayerTeamInformation(main, player.getName(), args[1]);
                        }
                    }
                } else {
                    if (args[0].equalsIgnoreCase("chat")){
                        if (Team.fromPlayer(main, player) != null){
                            ...
                            String message = MessageType.PLAYER_MESSAGE.setPlayer(player).setMessage(playerMessage[0]
                                    .substring(2)).setTeam(Team.fromPlayer(main, player)).build(main);
                            TeamChat.send(Team.fromPlayer(main, player), message);
                        }
                    }
                }
